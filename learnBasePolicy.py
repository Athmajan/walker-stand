...
domainName = "cheetah" # Name of a environment (set it to any Continous environment you want)
taskName = "run" # Name of a environment (set it to any Continous environment you want)
...
def process_state(state):
    if isinstance(state, OrderedDict):
        if 'position' in state and 'velocity' in state:
            orient = state['position']
            #height = state['height']
            velocity = state['velocity']
            out = np.concatenate((orient, velocity))
            return out
...
orientDim = obsSpec['position'].shape[0]
velocityDim = obsSpec['velocity'].shape[0]
input_dim = orientDim + velocityDim
